...
export function loadConfig() {
  ...
  saveConfig(cfg);
  const effective = { ...cfg };
  if (process.env.OPENBRIDGE_TOKEN && process.env.OPENBRIDGE_TOKEN.length >= 16) effective.token = process.env.OPENBRIDGE_TOKEN;
  if (process.env.OPENBRIDGE_PIN && /^\d{4}$/.test(process.env.OPENBRIDGE_PIN)) effective.pin = process.env.OPENBRIDGE_PIN;
  if (process.env.OPENBRIDGE_PORT) {
    const p = Number(process.env.OPENBRIDGE_PORT);
    if (Number.isInteger(p) && p > 0 && p < 65536) effective.port = p;
  }
  return effective;
}
